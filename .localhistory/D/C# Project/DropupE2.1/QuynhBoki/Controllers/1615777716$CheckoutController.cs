...
namespace QuynhBoki.Controllers
{
    public class CheckoutController : Controller
    {
        ...
        {
            if (Request.Cookies["QUYNHBOKI_CART"] != null)
            {
                var list_from_cookie = GetListFromCookie("QUYNHBOKI_CART");
        ...
        public List<ProductVariationInCartModel> GetListFromCookie(string name)
        {
            ...
            if (Request.Cookies["QUYNHBOKI_CART"] != null)
            {
                string objCartListString = HttpUtility.UrlDecode(Request.Cookies["QUYNHBOKI_CART"].Value.ToString());
            ...
            decimal amount_received, int? coupon_id)
        {
            ...
            if (coupon_id != 0)
            {
                coupon_provider.updateNumOfUse((int)coupon_id);
            }
        ...
        public async Task<ActionResult> SendEmailForAdmin(long order_id)
        {
            ...
            await SendEmail(email_receiver, order_id, order.create_datetime, (decimal)order.cost_of_shipping, order.payment_method.payment_method_description,
                ...
                order.delivery_address.last_name, da_detail, order.delivery_address.phone_number, order.order_note ,ContentEmailListProduct(order_id));
        ...
        public async Task<bool> SendEmail(string email_receiver, long order_id, DateTime order_create_datetime,
            ...
            string delivery_address_fullname, string delivery_address_detail, string delivery_address_phone_number, string order_note,
            ...
            string order_list_product)
        {
            ...
            content = content.Replace("{{customer_note}}", store_phone_number);
