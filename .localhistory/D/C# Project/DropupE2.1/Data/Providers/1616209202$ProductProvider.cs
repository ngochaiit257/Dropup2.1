    ...
    public class ProductProvider
    {
        ...
        public List<product> filterOnClientSiteElysien(string seo_alias, string search_string, decimal? min_price, decimal? max_price)
        {
            try
            {
                ...
                list_return = getByMoneyRange((decimal)min_price, (decimal)max_price, list_input)).Where(p => (p.product_name + p.supplier.supplier_name).ToLower().Contains(search_string.ToLower().ToList());
                return list_return;
            }
            catch
            {
